...
- All [`ref`](/reference/dbt-jinja-functions/ref) or [`source`](/reference/dbt-jinja-functions/source) references within the model must be included in the unit test configuration as`input`s otherwise you may encounter "node not found" errors during compilation.
